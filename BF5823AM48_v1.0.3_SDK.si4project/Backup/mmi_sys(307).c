...
void timer2_event_handler(void)
{
	...
	if((mmi_dq_rst_timer_event() != 0) && (SYS_STATUS_ENTER_SLEEP != state && SYS_STATUS_WAIT_FOR_ENTER_SLEEP != state) && (g_timer2_flag != 1) && (mmi_dq_ms_get_run_flag() == 0))
	{
		...
		if(state == SYS_STATUS_ADD_PWD || state == SYS_STATUS_DEL_PWD || state == SYS_STATUS_ADD_FP|| state == SYS_STATUS_DEL_FP|| state == SYS_STATUS_ADD_RFID|| state == SYS_STATUS_DEL_RFID)
		{
		...
		if(flag)
		{
			if(state >= SYS_STATUS_ADD_PWD && state <= SYS_STATUS_ADD_ADMIN_FP2 || state == SYS_STATUS_SYS_MENU)
			{
