...
static void mmi_dq_fds_read(uint8_t page, uint16_t addr, uint8_t* r_data ,uint16_t r_size)
{
	uint16_t i;
	eeprom_select(0);
	for(i = 0; i < r_size; i++)
	{
		r_data[i] = eeprom_read_byte(page,i+addr);//¶ÁµÚÒ»Ò³
	}
}

static RET_VAL mmi_dq_fds_write(uint8_t page, uint16_t addr, uint8_t* w_data ,uint16_t w_size)
{
	uint16_t i;
    ErrStatus ret;
	eeprom_erase_time(9);
	...
	eeprom_erase_page(page);//²Á³ý1K
	
	for(i=0;i<w_size;i++)//Ð´Ò»Ò³
	{
		ret = eeprom_write_byte(page,i+addr,w_data[i]);
		if(ret==ERROR) 
		{
			return RET_FAIL;
		}
...
{
	mmi_dq_fds_read(MID_FDS_FILE_SET, 0, (unsigned char *)&g_dq_fs_init_set, sizeof(mmi_fs_setting));
	if(FDS_INIT_LOCK_SUC == g_dq_fs_init_set.init_flag || FDS_INIT_APP_SUC == g_dq_fs_init_set.init_flag)
	{
		mmi_dq_fds_read(MID_FDS_FILE_PWD, 0, (unsigned char *)g_dq_fs_pwd, sizeof(mmi_fs_pwd)*MMI_DQ_FS_PWD_MAX_NUM);
		mmi_dq_fds_read(MID_FDS_FILE_FP, 0, (unsigned char *)g_dq_fs_fp, sizeof(mmi_fs_fp)*MMI_DQ_FS_FP_MAX_NUM);
		mmi_dq_fds_read(MID_FDS_FILE_RF, 0, (unsigned char *)g_dq_fs_rfid, sizeof(mmi_fs_rfid)*MMI_DQ_FS_RFID_MAX_NUM);
	}
	else
	{
		memset(g_dq_fs_pwd,0xFF,sizeof(g_dq_fs_pwd));
		mmi_dq_fds_write(MID_FDS_FILE_PWD, 0, (unsigned char *)g_dq_fs_pwd, sizeof(mmi_fs_pwd)*MMI_DQ_FS_PWD_MAX_NUM);

		memset(g_dq_fs_fp,0xFF,sizeof(g_dq_fs_fp));
		mmi_dq_fds_write(MID_FDS_FILE_FP, 0, (unsigned char *)g_dq_fs_fp, sizeof(mmi_fs_fp)*MMI_DQ_FS_FP_MAX_NUM);

		memset(g_dq_fs_rfid,0xFF,sizeof(g_dq_fs_rfid));
		mmi_dq_fds_write(MID_FDS_FILE_RF, 0, (unsigned char *)g_dq_fs_rfid, sizeof(mmi_fs_rfid)*MMI_DQ_FS_RFID_MAX_NUM);

		g_dq_fs_init_set.init_flag = FDS_INIT_INVALID;
		...
		mmi_dq_fds_write(MID_FDS_FILE_SET, 0, (unsigned char *)&g_dq_fs_init_set, sizeof(mmi_fs_setting));
	}
}
...
RET_VAL mmi_dq_fs_set_pwd(unsigned char *pwd,unsigned char size,fds_use_type type)
{
	...
	if(i<MMI_DQ_FS_PWD_MAX_NUM)
	{
		...
		mmi_dq_fs_pwd_string_to_byte(pwd,size,password);
		memcpy((char *)g_dq_fs_pwd[i].key_pwd,(const char *)password,4);
		g_dq_fs_pwd[i].flag = type;
		return	mmi_dq_fds_write(MID_FDS_FILE_PWD, 0, (unsigned char *)g_dq_fs_pwd,sizeof(mmi_fs_pwd)*MMI_DQ_FS_PWD_MAX_NUM);
...
RET_VAL mmi_dq_fs_set_fp(unsigned char fs_index,fds_use_type type)
{
	...
	return mmi_dq_fds_write(MID_FDS_FILE_FP, 0, (unsigned char *)g_dq_fs_fp, sizeof(mmi_fs_fp)*MMI_DQ_FS_FP_MAX_NUM);
}
...
RET_VAL mmi_dq_fs_set_rfid(unsigned char *sec_data, fds_use_type type)
{
	...
	if(i < MMI_DQ_FS_RFID_MAX_NUM)
	{
		...
		return mmi_dq_fds_write(MID_FDS_FILE_RF, 0, (unsigned char *)g_dq_fs_rfid, sizeof(mmi_fs_rfid)*MMI_DQ_FS_RFID_MAX_NUM);
	}
	return RET_FAIL;
}
...
RET_VAL mmi_dq_fs_del_rfid(unsigned char rfid_index)
{
	if(rfid_index < MMI_DQ_FS_RFID_MAX_NUM && g_dq_fs_rfid[rfid_index].index == rfid_index)
	{
		...
		memset(g_dq_fs_rfid[rfid_index].sec_data,0xFF,sizeof(g_dq_fs_rfid[i].sec_data));
		return mmi_dq_fds_write(MID_FDS_FILE_RF, 0, (unsigned char *)g_dq_fs_rfid, sizeof(mmi_fs_rfid)*MMI_DQ_FS_RFID_MAX_NUM);
	}
	return RET_FAIL;
}
...
RET_VAL mmi_dq_fs_set_open_mode(sys_open_mode mode)
{
	...
	return mmi_dq_fds_write(MID_FDS_FILE_SET, 0, (unsigned char *)&g_dq_fs_init_set, sizeof(mmi_fs_setting));
