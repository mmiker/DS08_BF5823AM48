...
void mmi_ms_pwd_opt_fun(uint8_t key_val)
{
	...
	else if((key_val == KEY_H) || (key_len == PWD_INPUT_MAX_LEN))
	{
		...
		{
			mmi_ms_pwd_init_var();
			mmi_dq_sys_show_message_with_id(STR_ID_PWD_LEN_68,0);
		} 
		else
		{
...
void mmi_ms_rfid_opt_fun(uint8_t rfid_val)
{
	...
	if(retval == RET_SUCESS)
	{
		...
		{
			mmi_dq_sys_show_message_with_id(STR_ID_RFCARD_EXIST,0);
		}
		else if(status == SYS_STATUS_DEL_RFID)
		{
			...
			if(retval == RET_SUCESS)
			{
				...
				else
				{
					...
					if(retval == RET_SUCESS)
					{
						retval = mmi_dq_rfid_delete(index);
						if(retval == RET_SUCESS)
						{
							mmi_dq_sys_show_message_with_id(STR_ID_DEL_RFCARD_SUCESS,0);
						}
						else
							mmi_dq_sys_show_message_with_id(STR_ID_DEL_FAIL,0);
					}
					else
						mmi_dq_sys_show_message_with_id(STR_ID_RFCARD_NOT_SAME,0);
					opt_time = OPT_ONE_TIME;
				}
			}
			else
				mmi_dq_sys_show_message_with_id(STR_ID_DEL_FAIL_RETRY,0);
		}
			...
			if(retval == RET_SUCESS)
			{
				...
				else
				{
					...
					if(retval == RET_SUCESS)
					{
						...
						if(retval == RET_SUCESS)
						{
							mmi_dq_sys_show_message_with_id(STR_ID_ADD_RFCARD_SUCESS,0);
						}
						else
							mmi_dq_sys_show_message_with_id(STR_ID_ADD_FAIL,0);
					}
					else
						mmi_dq_sys_show_message_with_id(STR_ID_RFCARD_NOT_SAME,0);
					opt_time = OPT_ONE_TIME;
				...
				mmi_dq_sys_show_message_with_id(STR_ID_ADD_FAIL_RETRY,0);
		}
		else
			mmi_dq_sys_show_message_with_id(STR_ID_RFCARD_NOT_EXIST,0);
	}
