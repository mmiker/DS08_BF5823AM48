...
void mmi_ms_fps_opt_fun(unsigned char fps_val)
{
	...
	if((status != SYS_STATUS_INPUT_FP && status != SYS_STATUS_ADD_FP && status != SYS_STATUS_DEL_FP && status != SYS_STATUS_ADD_ADMIN_FP1 && status != SYS_STATUS_ADD_ADMIN_FP2)||(status == SYS_STATUS_FM_MODE && STR_ID_FINGERPRINT != mmi_dq_factory_mode_get_test_project()))
		return;
...
void mmi_ms_reset_opt_fun(void)
{
	...
	else
	{
		if (RET_SUCESS == mmi_dq_fs_reset())
			mmi_dq_aud_play_with_id(AUD_ID_SYS_RESTORE_SUCCESS);
		else
