...
u8 M1_Example(u8 *uid)
{
	...
	u8 aucBlkBuf[18];
   	...
   	ret = MifareAuthenticate(uid, 4, 'A', aucPswA, 0);//每个扇区认证一次,第一次执行认证
    if(RETURN_OK == ret)
	{
		if(RETURN_OK == MifareOperations( 'r', 4, 4, aucBlkBuf))//读第4块数据 16byte
		{			
		   ret = RETURN_OK;
		}
		if(RETURN_OK == MifareOperations( 'r', 5, 5, aucBlkBuf))//读第5块数据 16byte
		{			
		   ret = RETURN_OK;
		}
		if(RETURN_OK == MifareOperations( 'r', 6, 6, aucBlkBuf))//读第6块数据 16byte
		{			
		   ret = RETURN_OK;
		}
	}
