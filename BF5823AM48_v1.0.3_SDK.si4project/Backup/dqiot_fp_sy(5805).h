...
#define FP_PWREN_ON		1
#define FP_PWREN_OFF	2
#endif
...
void SendCheck(unsigned short check);//发送校验和
 ...
 unsigned short PS_GetImage(void);// 探测手指，探测到后录入指纹图像存于 ImageBuffer
 unsigned short PS_GenChar(unsigned char BufferID);//将ImageBuffer中的原始图像生成指纹特征文件存于CHARBUFFER1或CHARBUFFER2
 unsigned short PS_Match(void);// 精确比对 CHARBUFFER1与CHARBUFFER2中的特征文件
 unsigned short PS_RegModel(void);//将CHARBUFFER1与CHARBUFFER2 中的特征文件合并生成模板，
 unsigned short PS_StoreChar(unsigned char BufferID,unsigned short PageID);//将 CHARBUFFER1 或 CHARBUFFER2 中的模板文件存到 PageID 号flash 数据库位置。
 unsigned short PS_DeletChar(unsigned short PageID,unsigned short N);//删除 flash 数据库中指定ID号开始的N个指纹模板
 unsigned short PS_HighSpeedSearch(unsigned char BufferID,unsigned short StartPage,unsigned short PageNum,unsigned short *p);//以CHARBUFFER1或CHARBUFFER2中的特征文件高速搜索整个或部分指纹库
 unsigned short  PS_Empty(void);// 删除 flash 数据库中所有指纹模板
 unsigned short PS_Connect(unsigned long *PS_Addr);//检查STM32与指纹模块的通信连接
