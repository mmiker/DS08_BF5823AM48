...
return :
	none
*/
static unsigned char mmi_Queue_GetNextID(unsigned char id)
{
	if(id < QUEUE_NUM)
		return id + 1;
	else
...
return :
	none
*/
static unsigned char mmi_Queue_GetPreID(unsigned char id)
{
	if(id > 0)
		return id - 1;
	else
...
return :
	none
*/
unsigned char mmi_OutQueue(Sys_MSG_Queue_M * que)
{
	unsigned char que_len = 0;
	unsigned char i = 0;
	...
	g_msg_que.head = mmi_Queue_GetNextID(g_msg_que.head);
	return 1;
}
